...
static void LoadCompositeRoute101Tiles(const u8 *tiles)
{
    ...
    u8 y;
    u8 i;
    u8 ALIGNED(4) tileGfx[TILE_SIZE_4BPP];
    u16 metatileId;
    const u16 *metatile;
...
static void LoadMappedRoute101Tiles(void)
{
    if (Route101_Layout.primaryTileset->isCompressed)
        DecompressDataWithHeaderWram(Route101_Layout.primaryTileset->tiles, sBattleOwDecompressionBuffer);
    else
        CpuCopy32(Route101_Layout.primaryTileset->tiles, sBattleOwDecompressionBuffer, NUM_TILES_IN_PRIMARY * TILE_SIZE_4BPP);
    CopyMappedRoute101Tiles(sBattleOwDecompressionBuffer, 0, NUM_TILES_IN_PRIMARY);
    LoadCompositeRoute101Tiles(sBattleOwDecompressionBuffer);

    if (Route101_Layout.secondaryTileset->isCompressed)
        DecompressDataWithHeaderWram(Route101_Layout.secondaryTileset->tiles, sBattleOwDecompressionBuffer);
    else
        CpuCopy32(Route101_Layout.secondaryTileset->tiles, sBattleOwDecompressionBuffer, (NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY) * TILE_SIZE_4BPP);
    CopyMappedRoute101Tiles(sBattleOwDecompressionBuffer, NUM_TILES_IN_PRIMARY, NUM_TILES_TOTAL - NUM_TILES_IN_PRIMARY);
}
