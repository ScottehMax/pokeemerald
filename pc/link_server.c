...
static void SendMatch(PcSocket socket, const struct Match *match, int player)
{
    ...
    packet.peerAddress = match->address[player ^ 1].sin_addr.s_addr;
    packet.peerPort = match->address[player ^ 1].sin_port;
    sendto(socket,
           (const char *)&packet,
           sizeof(packet),
           0,
                        ...
                        uint64_t now)
{
    ...
    if (nonce == 0 || packet->codeLength == 0 || packet->codeLength > PC_LINK_CODE_LENGTH)
        return;
...
int main(int argc, char **argv)
{
    ...
    while (sRunning)
    {
        ...
        if (size == sizeof(packet)
         ...
         && packet.version == PC_LINK_VERSION
         && packet.type == PC_LINK_PACKET_HELLO)
