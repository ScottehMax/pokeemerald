...
string generate_map_header_text(Json map_data, Json layouts_data) {
    ...
    text << "asset_ptr_align\n" << mapName << ":\n"
         << "\tasset_ptr " << json_to_string(layout, "name") << "\n";

    if (map_data.object_items().find("shared_events_map") != map_data.object_items().end())
        text << "\tasset_ptr " << json_to_string(map_data, "shared_events_map") << "_MapEvents\n";
    else
        text << "\tasset_ptr " << mapName << "_MapEvents\n";

    if (map_data.object_items().find("shared_scripts_map") != map_data.object_items().end())
        text << "\tasset_ptr " << json_to_string(map_data, "shared_scripts_map") << "_MapScripts\n";
    else
        text << "\tasset_ptr " << mapName << "_MapScripts\n";
    ...
    if (map_data.object_items().find("connections") != map_data.object_items().end()
     && map_data["connections"].array_items().size() > 0 && json_to_string(map_data, "connections_no_include", true) != "TRUE")
        text << "\tasset_ptr " << mapName << "_MapConnections\n";
    else
        text << "\tasset_ptr NULL\n";
...
string generate_groups_text(Json groups_data) {
    ...
    for (auto &key : groups_data["group_order"].array_items()) {
        ...
        for (Json &map_name : maps)
            text << "\tasset_ptr " << json_to_string(map_name) << "\n";
        text << "\n";
    }

    text << "\tasset_ptr_align\n" << "gMapGroups::\n";
    for (auto &group : groups_data["group_order"].array_items())
        text << "\tasset_ptr " << json_to_string(group) << "\n";
    text << "\n";
...
string generate_layout_headers_text(Json layouts_data) {
    ...
    for (auto &layout : layouts_data["layouts"].array_items()) {
        ...
        text << border_label << "::\n"
             ...
             << "\tasset_ptr " << border_label << "\n"
             << "\tasset_ptr " << blockdata_label << "\n"
             << "\tasset_ptr " << json_to_string(layout, "primary_tileset") << "\n"
             << "\tasset_ptr " << json_to_string(layout, "secondary_tileset") << "\n";
        if (version == "firered") {
